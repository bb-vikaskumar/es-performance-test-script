...
async function createRecords({docCount, batchSize}) {
    let sourceSpace = {
        ...
        c_sku_id: [...generateSeries({from: 1000000, count: SKUS_IN_EACH_SEARCH_REQ*SKUS_PER_DOC*10})],
    ...
    console.log('** allDocsLength: ', allDocsLength);
    for(let i=0; i<allDocsLength; i++) {
        ...
        for(let j=0; j<SKUS_IN_EACH_SEARCH_REQ; j++) {
            ...
            if(cloneDoc.c_sku_id.length !== SKUS_PER_DOC) {
                console.log("** issue here", {doc_after: cloneDoc.c_sku_id, j, SKUS_PER_DOC, CURRENT: cloneDoc.c_sku_id.length})
            }
...
async function fetchRecords({docCount, batchSize}) {
    ...
    if(ACTIVITY_QTY_TYPE == 'count') {
        for(let i=0; i<Math.max(docCount, SKUS_IN_EACH_SEARCH_REQ); i++) {
